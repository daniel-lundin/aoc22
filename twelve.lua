...
local function find_shortest_path(start_x, start_y)
	...
	return shortest_paths[goal_y][goal_x]
end

local shortest = 100000

for y, height_line in ipairs(mountain) do
	for x, height in ipairs(height_line) do
		if height == 1 then
			local distance = find_shortest_path(x, y)
			print('a at ' .. x .. ', ' .. y .. ' distance ' .. distance)
